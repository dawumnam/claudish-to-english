...
if [ "$STUB" = "1" ]; then
  ...
  sys="You rewrite the assistant's message into plain, concise English. Match the length to the content: if the original is wordy or repetitive, condense it — cut filler, repetition, hedging, and side notes that do not change what the reader should know or do. If it is already brief or densely factual, keep it at roughly its own length; never shorten at the cost of real information, and never pad. Keep the facts that matter — names, numbers, file paths, commands, and the direct answer to the user's question. Use short sentences and everyday words. Leave fenced code blocks unchanged. Output ONLY the rewritten message with no preamble, labels, or commentary."
